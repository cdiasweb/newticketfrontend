...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const processFiles = async (event: any) => {
      try {
        ...
        reader.onload = (e: any) => {
          ...
          if(fileZise > 8) {
            showMessage("Envio de imagens", "Uma imagem selecionada possui mais que 8MB, compacte a imagem usando um software online.", "alert");
            return;
          }
        ...
        console.error("Erro ao selecionar imagens: ", e);
        emit("error", "Erro ao selecionar imagens: " + e.message);
      }
    };
    ...
    const excluir = (image: imageUploadPreview): void => {
      if (image.imagePreview.idimage != undefined && image.imagePreview.idimage != -1) {
